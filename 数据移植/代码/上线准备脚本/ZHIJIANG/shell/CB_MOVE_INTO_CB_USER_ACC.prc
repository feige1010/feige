...
begin
 ...
 select distinct (substr(op.ncid,2) || substr(op.oid, 3)) as userId,
   ...
   from FIRMBANK_ZHIJIANG.t_account acc, FIRMBANK_ZHIJIANG.t_operator op,
       (select distinct auth.oid, auth_acc.acct_id
         from  FIRMBANK_ZHIJIANG.t_auth auth, FIRMBANK_ZHIJIANG.t_auth_account auth_acc
  ...
  union
   ...
   from FIRMBANK_ZHIJIANG.t_operator op,
        FIRMBANK_ZHIJIANG.t_account acc
