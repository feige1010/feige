...
select cid,
  ...
  from (select cst.cid,
          ...
          from PERBANK_ZHIJIANG.t_account acc,
               ...
               (SELECT DISTINCT cert.*, eb_cert.pb_no, eb_cert.cert_status
                  from  (SELECT * FROM COMM_ZHIJIANG.t_cert WHERE (dniso, id_cert) IN
                        (SELECT dniso, MAX(id_cert) FROM COMM_ZHIJIANG.t_cert cert GROUP BY dniso)) cert, EB_CERTIFICATE eb_cert
 ...
 where (cid, CRE_DATE) in
       (select acc.cid, max(cert.CRE_DATE) as CRE_DATE
          from COMM_ZHIJIANG.t_cert cert, PERBANK_ZHIJIANG.t_account acc
                   ...
                   FROM (SELECT DISTINCT T11.CID, T22.VALUE, T22.REG_DATE
                           FROM (SELECT DISTINCT CST.CID, ACC.ACCT_NO
                                ...
                                (SELECT FN.ACCT_NO,
                                        ...
                                        FA.VALUE,
                                        FA.REG_DATE
                                   FROM PERBANK_ZHIJIANG.T_ACCT_FUNC      FN,
                                        PERBANK_ZHIJIANG.T_ACCT_FUNC_ATTR FA
