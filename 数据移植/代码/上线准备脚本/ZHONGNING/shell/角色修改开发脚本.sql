select *
  from FIRMBANK_ZHONGNING.t_bizflow_cfg cfg
...
where auth.id_auth = rl.id_auth
  ...
  group by oid
            ...
            select cert.cci_cstno, cert.cci_userno, cert.cci_certid
              from FIRMBANK_ZHONGNING.t_auth auth, FIRMBANK_ZHONGNING.t_auth_role rl, CB_CERT_INF cert
             where auth.id_auth = rl.id_auth
               and auth.auth_status = '1'
             ...
             select auth.ncid, auth.oid
              from FIRMBANK_ZHONGNING.t_auth auth, FIRMBANK_ZHONGNING.t_auth_role rl
             where auth.id_auth = rl.id_auth
               and auth.auth_status = '1'
               and substr(auth.ncid, 2) = '0000010158'
               and rl.role_id = 'CHECKER';
...
(select distinct auth.ncid, role_id, auth.oid
 ...
 where auth.id_auth = rl.id_auth
   ...
   and auth.ncid in (select ncid
                      from FIRMBANK_ZHONGNING.t_firm
                     where firm_status != '0')
   and  oid in (select oid
                     from FIRMBANK_ZHONGNING.t_operator
...
select *
  from (select t1.oid as owner, count(his.id_current_step) as ownerNum
          from (select distinct auth.oid
               ...
               FIRMBANK_ZHONGNING.T_BF_HISTORY_STEP his
...
SELECT * FROM FIRMBANK_ZHONGNING.T_BF_HISTORY_STEP 
WHERE OWNER = '000022';
