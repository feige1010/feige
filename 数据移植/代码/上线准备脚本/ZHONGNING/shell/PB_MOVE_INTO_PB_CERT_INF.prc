...
select cid,
       ...
       to_char(CRE_DATE, 'yyyymmdd') as CRE_DATE,
       to_char(OVER_DATE, 'yyyymmdd') as OVER_DATE,
       '4' as STATUS, --激活状态
       (case MEDIA
         when '1' then
          '0' --非预制证书
  ...
  from (select cst.cid,
               ...
               cert.REF_CODE,
               cert.AUTH_CODE
          from PERBANK_ZHONGNING.t_account acc,
               PERBANK_ZHONGNING.t_customer cst,
               (SELECT DISTINCT cert.*, eb_cert.pb_no, eb_cert.cert_status
                  from  (SELECT * FROM COMM_ZHONGNING.t_cert WHERE (dniso, id_cert) IN
                        (SELECT dniso, MAX(id_cert) FROM COMM_ZHONGNING.t_cert cert GROUP BY dniso)) cert, EB_CERTIFICATE eb_cert
 ...
 where (cid, CRE_DATE) in
       (select acc.cid, max(cert.CRE_DATE) as CRE_DATE
          from COMM_ZHONGNING.t_cert cert, PERBANK_ZHONGNING.t_account acc
                   ...
                   FROM (SELECT DISTINCT T11.CID, T22.VALUE, T22.REG_DATE
                           FROM (SELECT DISTINCT CST.CID, ACC.ACCT_NO
                                ...
                                (SELECT FN.ACCT_NO,
                                        ...
                                        FA.VALUE,
                                        FA.REG_DATE
                                   FROM PERBANK_ZHONGNING.T_ACCT_FUNC      FN,
                                        PERBANK_ZHONGNING.T_ACCT_FUNC_ATTR FA
         ...
         (select cst.CID, fm.CTRL_BRANCH
            from (SELECT DISTINCT cst.*
                   from PERBANK_ZHONGNING.t_customer cst,
                        PERBANK_ZHONGNING.t_account acc,
                        COMM_ZHONGNING.t_cert cert
