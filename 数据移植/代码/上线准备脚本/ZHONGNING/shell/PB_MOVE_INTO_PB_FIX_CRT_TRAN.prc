  ...
  from (select regexp_replace(d.ROWID, '[/|+]', 'A') as UUID,
               (select cst.cid
                  from PERBANK_ZHONGNING.t_account acc,
                       ...
                       (select cert.*, eb_cert.pb_no
                          from COMM_ZHONGNING.t_cert cert, EB_CERTIFICATE eb_cert
               ...
               (case
                 when acct_type = '200' then
                  '00200100' --活期转定期
                 when acct_type = '215' or acct_type = '216' then
                  '00200102' --活转通知
               end) as BSNCODE,
               b.acct_name as 付款户名,
               b.branch as 付款开户行,
               ...
               a.card_no as 定期账号,
               a.card_no as 活期账号,
               decode(acct_type, '200', '01', '215', '14', '216', '14') as 定期类型,
               (case
         ...
         where a.main_acc_no = b.base_acct_no
           ...
           and acct_type in ('200', '215', '216')
           and b.User_id = 'EBUSER'
           and b.internal_key = c.internal_key
           and d.tran_type = 'EB25'
...
select (regexp_replace(d.ROWID, '[/|+]', 'A') || 'F2C') AS UUID,
       ...
       (case
          ...
          when acct_type = '215' or acct_type = '216' then '00200104'  --通知支取
       end) as BSNCODE,
       b.acct_name as 付款户名,
       b.branch as 付款开户行,
       b.acct_name as 收款方户名,
