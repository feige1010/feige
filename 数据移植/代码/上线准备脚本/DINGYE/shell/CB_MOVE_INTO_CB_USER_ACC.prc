...
begin
 ...
 select distinct (substr(op.ncid,2) || substr(op.oid, 3)) as userId,
   ...
   from FIRMBANK_DINGYE.t_account acc, FIRMBANK_DINGYE.t_operator op,
       (select distinct auth.oid, auth_acc.acct_id
         from  FIRMBANK_DINGYE.t_auth auth, FIRMBANK_DINGYE.t_auth_account auth_acc
