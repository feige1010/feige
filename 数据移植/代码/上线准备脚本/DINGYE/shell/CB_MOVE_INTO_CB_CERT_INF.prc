...
/**/
 insert into CB_CERT_INF
    ...
    select substr(op.NCID, 2) as NCID,
           ...
           '4' as STATUS, --激活状态
           nvl(start_date,to_char(cert.CRE_DATE, 'yyyymmdd')) as CRE_DATE,
           nvl(end_date,to_char(cert.OVER_DATE, 'yyyymmdd')) as OVER_DATE,
           cert.REF_CODE,
           cert.auth_code,
      ...
      from FIRMBANK_DINGYE.t_OPERATOR op,
           (select cert.*, eb_cert.pb_no,eb_cert.start_date, eb_cert.end_date
