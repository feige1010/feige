...
SELECT(CASE
       ...
       SRVRTID,
        ...
        (select op.oper_name from FIRMBANK_DINGYE.t_operator op where oid = ENTY.OWNER and rownum = 1) as oper_name,
        FUNC_GET_CB_TRANS_BSN(trans.NCID,TO_ACCT_ID) as BSNCODE,
        (CASE 
         WHEN trans.EXPECT_DATE is NULL 
         THEN '0'
         ELSE '1'
        END) as PRETRANFLAG,
        (CASE 
         WHEN trans.EXPECT_DATE IS NOT NULL 
         THEN to_char(EXPECT_DATE, 'yyyymmddhh24miss')
         ELSE null
        END) as EXPECT_DATE,
        '1103',
         ...
         '01' as CRY,
          factorage,
          (CASE WHEN trans.Intra_Bank = 'N' THEN
            '1'
            ELSE
            '0'
            END) AS TFL_TYPE
