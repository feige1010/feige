...
begin
 ...
 insert into cb_auth_def
  (ADF_ID,
   ...
   ADF_CHECKTYPE,
   ADF_SENDTYPE)
  select substr(NCID, 2) as ADF_ID,
         (case
         ...
         '0'
   from
     FIRMBANK_DINGYE.t_bizflow_cfg cfg
  ...
  (CASE
         ...
         '0' as CHECKTYPE,
         '0'
   from FIRMBANK_DINGYE.t_firm firm
   where firm_status != '0';
   ...
   --企业授权金额区间表，企业理财授权
     ...
     (CASE
        WHEN INSTR(NCID, '10',1 ,1) = 1 THEN
          '01' || substr(cfg.ncid, 4)
      ELSE
          '01' || substr(cfg.ncid, 2)
      END) as AAR_ID,
            '1',
            0.00,
            1000000000000000.00,
   ...
   --企业授权金额区间表，企业余额对账
     ...
     select
      (CASE
        WHEN INSTR(NCID, '10',1 ,1) = 1 THEN
          '011' || substr(firm.ncid, 5)
       ELSE
          '011' || substr(firm.ncid, 2)
       END) as AAR_ID,
            '1',
            0.00,
            1000000000000000.00,
 ...
 from
  ...
  from
   (select substr(firm.ncid,2) as ncid
    from FIRMBANK_DINGYE.t_firm firm, FIRMBANK_DINGYE.t_bizflow_cfg cfg
  ...
  select t_cst.ncid,
         ...
         '01',
    (CASE
        WHEN INSTR(t_cst.ncid, '00',1 ,1) = 1 THEN
          '01' || substr(t_cst.ncid,3)
       ELSE
          '01' || t_cst.ncid
     END)   as arf_id
  from
...
SELECT distinct
          (CASE
            WHEN INSTR(cif_cstno, '00',1 ,1) = 1 THEN
                '01' || substr(cif_cstno, 3)
          ELSE
                '01' || cif_cstno
          END) AS ADF_ID,
                '理财两步模式' AS ADF_ALIAS,
